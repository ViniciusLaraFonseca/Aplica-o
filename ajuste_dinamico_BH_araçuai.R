...
make_data_region <- function(cons_df, inst_df, subn_df, E_df, Y_df) {
  ...
  n_t   <- length(anos)
  x_mat <- array(NA_real_, dim = c(1L, n_t, 3L),
                 dimnames = list(NULL, anos, c("prenatal", "instrucao", "baixo_peso")))
  x_mat[1, , 1] <- as.numeric(cons_df[1, -1])
  x_mat[1, , 2] <- as.numeric(inst_df[1, -1])
  x_mat[1, , 3] <- as.numeric(subn_df[1, -1])
...
output_dir <- file.path(
  ...
  "resultados_dinamico_BH_ARACUAI"
)
