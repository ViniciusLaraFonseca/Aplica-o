# ==============================================================================
# ajuste_intercepto_temporal_K8.R
# Modelo Poisson log-linear com intercepto temporal i.i.d. — K = 8 clusters
#
# log(mu[i,t]) = beta0t[t] + log(E[i,t]) + log(epsilon[i])
#                + beta[1:p] %*% x[i,t,1:p]  (+ s[i]  se espacial)
#
# epsilon[i] = 1 - sum_{k=1}^K h[i,k] * gamma[k]
#
# Priors:
#   beta0t[t]  ~ N(0, 1)  i.i.d. para cada t = 1,...,T
#   beta[j]    ~ N(0, 1)
#   gamma[1]   ~ Unif(0, 0.1)
#   gamma[k]   ~ Unif(0, 1 - sum(gamma[1:(k-1)]))   k = 2,...,K
#   sigma_s    ~ Half-t(0, 1, 1)   [apenas espacial]
#   tau_s      = 1/sigma_s^2
#   s          ~ ICAR(tau_s)        [apenas espacial]
#
# K = 8  (lido automaticamente de _dataCaseStudy_K8.R)
# Inicialização: gamma[1] no ponto médio Unif(0, 0.1); camadas 2..8 com
#   pesos decrescentes no simplex residual; verificação formal das restrições.
# Estrutura, diagnósticos e estilo de saída seguem ajuste_gamma_temporal_K8.R
# ==============================================================================
...
# ==============================================================================
# 2. REGIÕES DE INTERESSE  (3 por cluster → até 24 regiões × 23 tempos)
# ==============================================================================
...
# ==============================================================================
# 4. INICIALIZAÇÕES — K = 8
# ==============================================================================
#
# gamma[1] ~ Unif(0, 0.1):  ponto médio = 0.05
#   Cadeia 1: g1 = 0.060  (acima do médio → diversidade)
#   Cadeia 2: g1 = 0.045  (abaixo do médio → diversidade)
# gamma[2..K]: pesos decrescentes no simplex residual.
#   Soma por cadeia < 1 garantida; todas as restrições verificadas.
# ==============================================================================

set.seed(123)

.init_gamma_vec <- function(g1, K) {
  pesos <- seq(0.08, 0.02, length.out = K - 1)
  pesos <- pesos / sum(pesos)
  c(g1, (1 - g1) * 0.5 * pesos)
}

gamma_init1 <- .init_gamma_vec(0.060, K)
gamma_init2 <- .init_gamma_vec(0.045, K)

stopifnot(
  "gamma_init1 soma < 1"      = sum(gamma_init1) < 1,
  "gamma_init2 soma < 1"      = sum(gamma_init2) < 1,
  "gamma_init1[1] em (0,0.1)" = gamma_init1[1] > 0 & gamma_init1[1] < 0.1,
  "gamma_init2[1] em (0,0.1)" = gamma_init2[1] > 0 & gamma_init2[1] < 0.1
)
cat("Inicializações de gamma verificadas: OK\n")
cat(sprintf("  soma gamma_init1 = %.4f | soma gamma_init2 = %.4f\n",
            sum(gamma_init1), sum(gamma_init2)))
...
run_model <- function(model_type, output_dir) {
  ...
  code_spatial <- nimbleCode({
    ...
    # Pesos de cluster (simplex aninhado, K=8)
    gamma[1] ~ dunif(a_unif, b_unif)
    for (k in 2:K) gamma[k] ~ dunif(0, 1 - sum(gamma[1:(k-1)]))
  ...
  # gamma amostrado em bloco (AF_slice captura restrições do simplex aninhado)
  ...
  ggsave(
    ...
    ggplot(epsilon_summary, aes(x = Region, y = Mean, color = factor(Cluster))) +
      geom_point(size = 0.9) +
      geom_errorbar(aes(ymin = Lower, ymax = Upper), width = 0.3, linewidth = 0.3) +
      scale_color_discrete(name = "Cluster") +
      theme_bw() +
      labs(title = paste("Epsilon posterior (", model_type, ") — K=8"),
  ...
  make_label <- function(regs) {
    setNames(
      sprintf("Reg %d (C%d)\ne=%.3f", regs, cluster_ids[regs],
              epsilon_summary$Mean[match(regs, epsilon_summary$Region)]),
      as.character(regs)
    )
  }
  
  # ── 5g. Sumário de gamma (K-vetor fixo) ──────────────────────────────────────
  
  gamma_summary <- do.call(rbind, lapply(seq_len(K), function(k) {
    nm  <- gamma_names[k]; sv <- samples_mat[, nm]; hpd <- safe_hpd(sv)
  ...
  # ── 5j. tau_s e s[i] (apenas espacial) ───────────────────────────────────────
  ...
  # ── 5k. mu para regiões selecionadas ─────────────────────────────────────────
  ...
  # Parâmetros estruturais: beta + gamma (+ tau_s)
  params_struct <- c(beta_names, gamma_names,
                     if (is_spatial) "tau_s" else character(0))
  # Diagnóstico completo inclui beta0t
  all_diag <- c(beta0t_names, params_struct)
  ...
  # ACF de beta0t — painel separado por tempo (padrão ouro)
  ...
  tibble(
    ...
    Rhat_gamma_max  = max(Rhat_struct[gamma_names], na.rm = TRUE),
...
output_dir <- file.path(
  ...
  "resultados_intercepto_temporal_K8"
)
