# ==============================================================================
# ajuste_lambda_ffbs_gamma_proporcional_kappa_slice.R
#
...
#   kappa[1] = 1;  kappa[k] = kappa[k-1] + incr[k-1],  incr[k] ~ Unif(0,M_incr)
#   dconstraint(kappa[K]*(L_max+delta_g) < 1)  garante epsilon > 0
#
# CORREÇÃO em relação à versão anterior:
#   incr[k] amostrados com slice INDIVIDUAL (não AF_slice em bloco)
#   → mais robusto a dconstraint rígido; evita cadeias presas.
#
# ADIÇÃO (acf_diagnostics.csv):
#   Seção 6q agora grava acf_diagnostics.csv com ESS, Rhat e lags para
#   todos os parâmetros monitorados — necessário para a tabela de diagnósticos
#   do relatório Rmd.
# ==============================================================================
...
gamma_1_init1 <- L_seq
gamma_1_init2 <- pmin(pmax(L_seq + rnorm(n_times, 0, delta_g / 4),
                           L_seq - delta_g + 1e-4),
                      L_seq + delta_g - 1e-4)
...
inits_nonspatial_1 <- list(
  lambda  = lambda_init,        beta = rep(0, p),
  gamma_1 = gamma_1_init1,      incr = incr_init1
)
...
run_model <- function(model_type, output_dir) {
  ...
  code_spatial <- nimbleCode({
    for (t in 1:n_times) { lambda[t] ~ dgamma(shape = a0, rate = b0) }
    for (j in 1:p)       { beta[j]   ~ dnorm(0, sd = 1) }
    for (t in 1:n_times) { gamma_1[t] ~ dunif(L[t] - delta_g, L[t] + delta_g) }
  ...
  model$calculate()   # força cálculo dos nós determinísticos antes de compilar
  Cmodel <- compileNimble(model)
  
  conf <- configureMCMC(model)
  
  # lambda: FFBS Gama-conjugado
  ...
  # incr: slice INDIVIDUAL — mais robusto a dconstraint do que AF_slice em bloco
  ...
  # ── 6h. Epsilon posterior ───────────────────────────────────────────────────
  n_draw      <- nrow(samples_mat)
  kappa_draws <- samples_mat[, kappa_names, drop = FALSE]
  h_kappa     <- h_mat %*% t(kappa_draws)
  
  epsilon_draws <- array(NA_real_, dim = c(n_draw, N_regions, n_times))
  for (t in seq_len(n_times)) {
    g1_t <- samples_mat[, gamma1_names[t]]
    epsilon_draws[, , t] <- t(1 - sweep(h_kappa, 2, g1_t, `*`))
  }
  ...
  eps_by_cluster <- do.call(rbind, lapply(1:K, function(k) {
    ...
    do.call(rbind, lapply(1:n_times, function(t) {
      sv  <- as.vector(epsilon_draws[, regs_k, t]); hpd <- safe_hpd(sv)
  ...
  # Parâmetros para diagnóstico completo (inclui temporais para ACF individual)
  ...
  ggsave(file.path(scenario_dir, "acf_lambda.png"),
         ggplot(acf_lambda_df, aes(x = Lag, y = ACF)) +
           ...
           labs(title    = paste("ACF de lambda[t] (", model_type, ")"),
                subtitle = "FFBS: autocorrelação esperada baixa"),
         width = 14, height = 10)
  ...
  ggsave(file.path(scenario_dir, "acf_gamma1.png"),
         ggplot(acf_g1_df, aes(x = Lag, y = ACF)) +
           ...
           labs(title    = paste("ACF de gamma_1[t] (", model_type, ")"),
                subtitle = "RW: autocorrelação esperada moderada"),
         width = 14, height = 10)
  ...
  ggsave(file.path(scenario_dir, "acf_incr_kappa.png"),
         ggplot(acf_incr_df, aes(x = Lag, y = ACF)) +
           ...
           labs(title    = paste("ACF dos incrementos incr[k] (", model_type, ")"),
                subtitle = "Slice individual: autocorrelação esperada baixa"),
         width = 8, height = 5)
  ...
  # ── 6q-EXTRA. acf_diagnostics.csv  ──────────────────────────────────────────
  # ADIÇÃO: grava tabela consolidada de ESS, Rhat e lags para todos os
  # parâmetros estocásticos — necessário para o relatório Rmd.
  # Parâmetros cobertos: beta, incr, tau_s (se espacial), lambda, gamma_1.
  all_diag_params <- c(
    beta_names,
    incr_names,
    lambda_names,
  ...
  acf_results <- do.call(rbind, lapply(all_diag_params, function(nm) {
  ...
  cat(sprintf("[%s] acf_diagnostics.csv gravado (%d parâmetros).\n",
              model_type, nrow(acf_results)))
...
resultados_m9 <- parLapply(cl, model_types, function(m) run_model(m, output_dir))
stopCluster(cl)

# ==============================================================================
# 8. CONSOLIDAÇÃO
# ==============================================================================
resumo <- bind_rows(resultados_m9)
write_csv(resumo, file.path(output_dir, "resumo_comparativo.csv"))
cat("\n=== RESUMO COMPARATIVO (M9) ===\n"); print(resumo)
...
if (nrow(lambda_all) > 0) {
  ggsave(file.path(output_dir, "lambda_comparativo.png"),
         ggplot(lambda_all, aes(x = Time, y = Mean, color = model, fill = model)) +
           ...
           labs(title = "lambda[t]: espacial vs. não-espacial (M9)",
                subtitle = paste0("Gama-DLM  |  w = ", w),
                x = "Tempo", y = expression(lambda[t]),
...
if (nrow(kappa_all) > 0) {
  ggsave(file.path(output_dir, "kappa_comparativo.png"),
         ggplot(kappa_all, aes(x = factor(k), y = Mean, color = model)) +
           ...
           labs(title = "kappa[k]: espacial vs. não-espacial (M9)",
                x = "Cluster k", y = expression(kappa[k]), color = "Modelo"),
         width = 7, height = 5, dpi = 300)
}

# Comparativo epsilon e mu
for (arq in c("epsilon_comparativo.png", "mu_comparativo.png")) {
  arq_nm <- str_remove(arq, "\\.png$")
  spatial_path     <- file.path(output_dir, "spatial",     str_replace(arq, "comparativo", "painel"))
  nonspatial_path  <- file.path(output_dir, "non_spatial", str_replace(arq, "comparativo", "painel"))
  eps_all <- lapply(model_types, function(m) {
    path <- file.path(output_dir, m, str_replace(arq, "comparativo",
                                                 ifelse(str_detect(arq, "epsilon"), "painel_epsilon", "painel_mu")))
    if (!file.exists(path)) return(NULL)
    read_csv(file.path(output_dir, m,
                       ifelse(str_detect(arq, "epsilon"),
                              "epsilon_cluster_summary.csv", "mu_selected.csv")),
             show_col_types = FALSE) |> mutate(model = m)
  }) |> bind_rows()
  
  if (nrow(eps_all) > 0) {
    # gráfico unificado
    p_uni <- ggplot(eps_all, aes(x = Time, y = Mean, color = model, fill = model)) +
      geom_ribbon(aes(ymin = Lower, ymax = Upper), alpha = 0.12, color = NA) +
      geom_line(linewidth = 0.8) +
      facet_wrap(~ if ("k" %in% names(eps_all)) k else Region,
                 ncol = 2, scales = "free_y") +
      scale_x_continuous(breaks = 1:n_times, labels = anos_label) +
      theme_bw(base_size = 12) +
      theme(axis.text.x = element_text(angle = 45, hjust = 1),
            legend.position = "bottom") +
      labs(title = paste0(arq_nm, ": espacial vs. não-espacial (M9)"),
           x = "Tempo", y = "Valor", color = "Modelo", fill = "Modelo")
    ggsave(file.path(output_dir, arq), p_uni, width = 10, height = 8, dpi = 300)
  }
}
